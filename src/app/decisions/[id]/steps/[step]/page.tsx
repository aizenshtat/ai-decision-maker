...
export default function DecisionStep() {
  ...
  const renderField = (field: Field) => {
    ...
    switch (field.type) {
      case 'matrix':
        const rowOptions = field.dependencies && 'rows' in field.dependencies
          ? getOptionsFromPreviousStep(`${field.dependencies.rows.step}.${field.dependencies.rows.field}.${field.dependencies.rows.use}`)
          : [];
        const columnOptions = field.dependencies && 'columns' in field.dependencies
          ? getOptionsFromPreviousStep(`${field.dependencies.columns.step}.${field.dependencies.columns.field}.${field.dependencies.columns.use}`)
          : [];
      ...
      case 'select':
        const options = field.dependencies && 'step' in field.dependencies
          ? getOptionsFromPreviousStep(`${field.dependencies.step}.${field.dependencies.field}.${field.dependencies.use}`)
          : [];
    ...
    <div className="max-w-2xl mx-auto mt-10 card">
      ...
      <form onSubmit={(e) => handleSubmit(e, true)} className="space-y-6">
        {stepData.fields.map((field) => (
          <div key={field.name} className="bg-gray-50 p-6 rounded-lg form-group">
            {renderField(field)}
